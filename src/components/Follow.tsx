...
const AvatarLink: React.FC<AvatarLinkProps> = ({ user }) => {
    ...
    return (
        <AccessibleRouterLink 
            ...
            <Avatar crossOrigin="anonymous" $size="sm" alt={`${user?.login || "User"}'s Avatar`} src={user?.avatar_url as string} />
        </AccessibleRouterLink>
    );
}
...
export const Follow: React.FC = () => {
    ...
    return (
        <div className="overflow-x-auto flex flex-row space-x-2 items-center justify-center">
            ...
            <span data-tut="tour-follow" className="md:flex hidden flex flex-row space-x-2">
